...
def judge(obs):
    ...
    r_b_distance2 = np.linalg.norm(obs["Escaper"][:2]-obs["Catcher2"][:2])      
    r_g_distance = np.linalg.norm(obs["Escaper"][:2]-obs["Goal"][:2])     
    print(f"距离终点{r_g_distance}m")

    if r_b_distance < 5 or r_b_distance2 < 5:
...
if __name__ == '__main__':
    ...
    car_name= ["Escaper","Catcher","Catcher2"]
    car_manul_control_flag = [True,False,False]
    car_origin_pos = [[0,0,0],[65,113,0],[-65,37,0]]
    car_agent = {}
    record_agent = {}
    # 初始化智能体
    ...
    action["Catcher2"] = {"throttle": 1,"steering": 0,"brake": 0}
    car_agent["Catcher2"].do_action(action["Catcher2"])
    ...
    while not done:
        if pre_obs:
            ...
            car_agent["Catcher"].do_action(action["Catcher"])
            action["Catcher2"]["steering"] = Propotional_navi(cur_obs, pre_obs, "Catcher2", 100)
            car_agent["Catcher2"].do_action(action["Catcher2"])
